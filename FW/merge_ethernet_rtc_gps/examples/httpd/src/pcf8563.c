...
/* Example code to talk to a PCF8563 Real Time Clock module 

   Connections on Raspberry Pi Pico board, other boards may vary.

   GPIO PICO_DEFAULT_I2C_SDA_PIN (On Pico this is 4 (physical pin 6)) -> SDA on PCF8563 board
   GPIO PICO_DEFAULT_I2C_SCK_PIN (On Pico this is 5 (physical pin 7)) -> SCL on PCF8563 board
   3v3(physical pin 40) -> VCC on PCF8520 board
...
uint8_t pcf8563_get_all_data(uint8_t* data, uint8_t base_address, uint8_t n_bytes){
    ...
    if(PICO_ERROR_GENERIC == i2c_write_blocking(i2c0, RTC_PCF8563_I2C_ADR, &base_address, 1, true)) //don't send stop after write op
        ...
        i2c_read_blocking(i2c0, RTC_PCF8563_I2C_ADR, data, n_bytes, false); //send stop after reading
        return 1;
    }
}
*/
